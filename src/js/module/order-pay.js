...
define('order-pay',['ajax', 'member', 'popup', 'widget', 'mvvm'],function (require, exports) {
    ...
    var order_detail_url, order_pay_url, pay_successfully_url;
    ...
    var vmData = MVVM.define('order-pay', {
        ...
        express_type: 0, 	                                             // 配送方式:0-普通快递,1-顾客自提,2-两小时达,3-当日达
        ...
        back: function(url,type) {
            ...
            if( vmData.is_pay_alipay && type == 1){
                ...
                var str_time = vmData.express_type == 2|| vmData.express_type == 3 ? '30分钟':'48小时' ;
                popup.confirm('是否取消支付<br>下单后'+ str_time  +'内未支付成功，订单将被取消，请尽快完成支付。', function() {
                    window.location = url;
                }, {
                    cancelText: '继续支付',
                    confirmText: '取消支付'
            ...
            if(!vmData.is_from_wechat && !vmData.is_global){
                ajax({
                    ...
                    data: {
                        ...
                        return_url: pay_successfully_url,  // 同步跳转地址
                        show_url: seajs.data.cwd + 'user-order.html#all',   // 订单列表页
                        token: member.token
                    },
                    dataType: 'json',
                    beforeSend: function () {
        ...
        // 银联支付
        other: function() {
            if(!vmData.is_global && !vmData.is_from_wechat) {
                ajax({
                    type: 'post',
                    url: config.url + 'wap/payment/baiyangpay_unified_order',
                    data: {
                        order_id: vmData.order_id,                          // 订单id
                        success_url: pay_successfully_url,  // 同步跳转地址
                        token: member.token
                    },
                    dataType: 'json',
                    beforeSend: function () {
        ...
        chose_pay_way: function(type) {
            ...
            if( type == 1 && !vmData.is_global){
                // 支付宝支付
                vmData.is_pay_alipay = true ;
                zfbPay();
            } else if( type == 2 && !vmData.is_global){
                // 微信支付
                vmData.is_pay_wechat = true ;
                wxPay();
            } else if(type == 3 && vmData.is_global){
                // 易票联微信支付
                vmData.is_pay_wechat = true ;
                yplWxPay();
            }
        ...
        // 是否显示易票联微信H5支付
        show_wx_h5: !(platform.isFromBaiYangApp&&platform.isFromAndroid&&platform.byAppVersion <= '3.1.3'),

        // 易票联支付
        ypl_pay: function(type) {

            // unionpaywap:银联在线支付WAP版,wxpay:银联在线支付WAP版  wxH5Pay 易票联微信H5支付
            var pay_id = ['wxpay', 'unionquickpay', 'wxH5Pay'][type];
            ...
            if(type == 2){
                ...
                // 支付成功跳转到支付成功页
                return_url = pay_successfully_url;
            }
                        ...
                        if(data && data.code == 200){
                            ...
                            if(data.data.is_pay == 1){
                                ...
                                popup.success('支付成功',function() {
                                    window.location.replace(pay_successfully_url);
                                });
    ...
    // 易票联查询订单是否支付成功
    var queryOrderIsPay = function(order_id) {
        ajax({
            type: 'POST',
            url: config.url + 'wap/payment/epaylinks_chkPayRes',
            data: {
                token : member.token,
                order_id : order_id
            },
            ...
            success: function(data) {
                ...
                if(data && data.code == 200 && String(data.data) == 'true'){
                    // 跳转到订单详情
                    window.location = pay_successfully_url;
                } else {

                    // 轮询订单状态
                    setTimeout(function() {
                        queryOrderIsPay(order_id);
                    }, 1000);
        ...
        member.onLogin(function() {
            ...
            member.getOrderDetail({
                ...
                if(data && data.code == 200 && data.data){
                    ...
                    order_detail_url = seajs.data.cwd + 'user-order-detail.html?id=' + data.data.orderInfo.order_sn + '&is_global=' + data.data.is_global;
                    order_pay_url = seajs.data.cwd + 'order-pay.html?order_id=' + data.data.orderInfo.order_sn + '&is_global=' + data.data.is_global;
                    pay_successfully_url = seajs.data.cwd + 'order-submit-successfully.html?order_id=' + data.data.orderInfo.order_sn + '&is_global=' + data.data.is_global;
                    ...
                    if(data.data.orderInfo.order_status == 'paying') {
                        ...
                        vmData.is_global = data.data.is_global == 1;
                        vmData.express_type = data.data.orderInfo.express_type ;

                        // 如果是易票联跨境支付，则查询订单是否已经支付
                        if(!platform.isFromWx && vmData.is_global) {
                            queryOrderIsPay(data.data.orderInfo.order_sn);
                        }

                    } else {
                        // 跳转到订单详情
                        window.location = order_detail_url;
                    }

                } else {
                    popup.error(data ? data.message : '订单参数错误！', function() {
                        window.location = 'user-order.html';
                    });
                }
            });
